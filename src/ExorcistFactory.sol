...
import {Exorcist} from "./Exorcist.sol";

contract ExorcistFactory {
    ...
    function deployExorcist(address _legionAddress) external returns (address _exorcist) {
                    ...
                    bytes32(
                        keccak256(
                            abi.encodePacked(
                                ...
                                keccak256(bytes.concat(type(Exorcist).creationCode, abi.encode(_legionAddress)))
                            )
                        )
                    )
                )
        ...
        if (_exorcist != address(new Exorcist{salt: bytes32(uint256(uint160(_legionAddress)))}(_legionAddress))) {
